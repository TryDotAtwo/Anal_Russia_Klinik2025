...
class ClinicalAnalyzer:
    ...
    async def load_clinical_recommendations(self):
        ...
        # Встраиваем метаданные в каждую клиническую рекомендацию перед сохранением,
        # чтобы они были доступны на этапе анализа и в итоговом JSON/CSV.
        for kr_key, kr_data in self.clinical_recommendations.items():
            kr_data["metadata"] = self.metadata.get(kr_key, {})
    ...
    async def load_from_json(self):
        ...
        try:
            ...
            # Гарантируем, что при загрузке из JSON метаданные также будут встроены.
            for kr_key, kr_data in self.clinical_recommendations.items():
                if "metadata" not in kr_data:
                    kr_data["metadata"] = self.metadata.get(kr_key, {})
